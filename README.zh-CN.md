...
**设计原则：** `azure-functions-scaffold` 负责项目生成与模板展开。它不提供运行时库 —— 运行时行为归属于 [`azure-functions-openapi`](https://github.com/yeongseon/azure-functions-openapi-python)（API 文档与规范生成）、[`azure-functions-validation`](https://github.com/yeongseon/azure-functions-validation-python)（请求/响应校验）和 [`azure-functions-langgraph`](https://github.com/yeongseon/azure-functions-langgraph-python)（LangGraph 运行时暴露）。
